/**
 * Code generator for S-expression-style DEX code
 */ 
...
public class SExpStyleCodeGenerator implements CodeGenerator {
    ...
    while( classIterator.hasNext() ) {
     ...
     // if( dexClassDefsBlock.getDexAnnotationParser( classidx ) != null )
      ...
      for( int i = 0 ; i < dexClassDefsBlock.getDirectMethodsFieldsSize( classidx ) ; ++i ) {
        ...
        // why I deleted the method annotation before???
      ...
      for( int i = 0 ; i < dexClassDefsBlock.getVirtualMethodsFieldsSize( classidx ) ; ++i ) {
        ...
        addMethodAnnotation( ps,
            dexClassDefsBlock.getVirtualMethodShortName( classidx, i ), 
        		//SExpHelpers.LTypeToSXType(dexClassDefsBlock.getVirtualMethodShortName( classidx, i )),
  ...
  public void renderPackedSwitch( 
    ...
    currentOutput.println( "    "+defaultLabelName+") ; default" );
  }
  ...
  public void openDataArray( String label ) throws IOException {
    renderLabel(label);
    currentOutput.println( "  (data-array" );
  }
  ...
  public void writeTryCatchBlockWithFinallyType(
		    int typeidx,
		    String startLabel, 
      String endLabel, 
      String exception,
      String handlerLabel ) throws IOException {
	String tail =  exception+
    " from "+
    startLabel+
    " to "+
    endLabel+
	...
	if(typeidx > 0 )
currentOutput.println( "  (catch "+ tail);
	else 
		currentOutput.println( "  (catchall  "+ tail);
}
  ...
  public void writeLocalVariable( int regNum,
    ...
    currentOutput.println( "(var "+
        ...
        " "+
        //variableType
        SExpHelpers.LTypeToSXType(variableType)+
        " from "+
        startOffsetLabel+
        " to "+
        endOffsetLabel + ")");
...
currentOutput.println();
 ...
 /* public void writeClassAnnotation( int classIdx, 
    ...
    currentOutput.println( " (annotation "+
        getVisibility( visibility )+
        " "+
        SExpHelpers.LTypeToSXType( type ) );
    for( int i = 0 ; i < parmNames.size() ; ++i ) {
      Object o = parmValues.get( i );
      currentOutput.println( "  ("+
          parmNames.get( i ) +
          " "+
          SExpHelpers.LTypeToSXType( DexEncodedArrayParser.getTypeString( o ) )+
          " "+
          o.toString()+
          ")" );
    }
  ...
  private static final int 	REVISIT_LIMIT = 100;
	...
	private void generateMethodBody(String methodName, long pos,
			PrintStream ps, int classidx, int methodidx, boolean direct)
		...
		ArrayList parmRegs = direct ? dexClassDefsBlock
				.getDirectMethodParameterOffsets(classidx, methodidx, regSize)
				: dexClassDefsBlock.getVirtualMethodParameterOffsets(classidx,
						methodidx, regSize);
		int accessFlags = direct ? dexClassDefsBlock.getDirectMethodAccess(
				classidx, methodidx) : dexClassDefsBlock
				.getVirtualMethodAccess(classidx, methodidx);
		HashMap<Integer, String> initRegMap = new HashMap<Integer, String>();
		if ((accessFlags & DexClassDefsBlock.ACCESS_STATIC) == 0) {
			int thisReg = 0;
			if (parmRegs.size() < 2) // no parameters - "this" is in the last
										// register
				thisReg = regSize - 1;
			else
		...
		for (int i = 0; i < parmRegs.size(); i += 2, ++parmctr) {
			ps.println("  ; parameter[" + parmctr + "] : v"
					...
					+ (SExpHelpers.LTypeToSXType((String) parmRegs.get(i + 1)))
					+ ")");
			initRegMap.put((Integer) parmRegs.get(i), DexInstructionParser
					.convertJavaTypeToInternal((String) parmRegs.get(i + 1)));
		}
		...
		// First pass: discover just the labels and the code areas. The trace
		// disassembler
		...
		// This map stores reg trace strings (suitable for displaying to the
		// user)
		// per locations (-r flag)
		HashMap<Long, String> regTraceMap = null;
		// This map stores the exception block start addresses and the
		...
		// This map stores the counter, how many times a certain distinguished
		// location was visited.
		// This protects about endless loops when the regmap solution does not
		// converge
		...
		// Process the try-catch blocks if any. Pushes any exception handlers to
		// the visit stack
		if (DEBUG_FLOW)
			System.out.println("Flow: about to process try-catch blocks");

		if (dexMethodHeadParser.getTriesSize() != 0)
			processTryCatchBlock(methodName, instructionParser,
					dexMethodHeadParser, visitStack, initRegMap,
		...
		if (dexMethodHeadParser.getDebugOffset() != 0L) {
			debugInfoParser = parseDebugInfoBlock(instructionParser,
					dexMethodHeadParser);
			if (regTraces != null) {
				initializeRegTrace(regTraces, debugInfoParser,
						dexMethodHeadParser);
				instructionParser.setRegTraces(regTraces);
			}
		}
		instructionParser.setFilePosition(dexMethodHeadParser
				.getInstructionBase());

		instructionParser.setPass(false);
		instructionParser.setRegisterMap((HashMap<Integer, String>) initRegMap
				.clone());
		do {
			long filePos = instructionParser.getFilePosition();
			if (DEBUG_FLOW)
				System.out
						.println("Flow: about to enter block parsing, file pos: 0x"
								+ Long.toHexString(filePos));
			while (filePos < endPos) {
				filePos = instructionParser.getFilePosition();
				if (DEBUG_FLOW)
					System.out.println("Flow: block parsing, file pos: 0x"
							+ Long.toHexString(filePos));
				if (DEBUG_REGTRACE)
				...
				// - If we have been here but there is no saved register map
				// here, continue.
				// - If we have been here and there is saved register map but
				// the overrun counter exceeds the limit, break
				// the analysis
				// - Otherwise if the register consistency check indicates that
				// we should continue, do it.
				...
				if (haveBeenHere) {
					...
					HashMap<Integer, String> savedRegMap = registerMaps
							.get(posObj);
					if (DEBUG_REGMAPS)
						System.out.println("regMaps: 0x"
								...
								+ Long.toHexString(filePos) + "; regmap: ["
								+ savedRegMap + "]");
					HashMap<Integer, String> currentRegMap = (HashMap<Integer, String>) instructionParser
							.getRegisterMap();
					// The location is target of a jump instruction but no
					// register map has been saved.
					...
					if (registerMaps.containsKey(posObj)
						...
						if (!overrunCheck(posObj, overrunCounter)) {
							if (DEBUG_REGMAPS)
								System.out.println("regMaps: overrun at 0x"
										+ Long.toHexString(filePos));
							break;
						}
				...
				// Check if an exception block is starting here. If so, save the
				// register maps for the handler(s)
				// Also, if there is a saved register map for this location,
				// restore the register map from the
				// saved version
				if (exceptionHandlerEntryPointList != null) {
					if (DEBUG_FLOW)
						System.out.println("Flow: handleRegMaps, file pos: 0x"
								+ Long.toHexString(filePos));
					handleRegMaps(methodName, exceptionHandlerEntryPointList,
							instructionParser);
				}
				// Insert debug variables into the register set to handle the
				// case when
				// the debug variable goes into scope ...
				if (DEBUG_FLOW)
					System.out.println("Flow: before parse");
				instructionParser.parse();
				if (DEBUG_FLOW)
					System.out.println("Flow: after parse");
				// Now handle the case when the debug variable goes out of
				// scope.
				// Save the register trace after the instruction if register
				// tracing is enabled
				if ((regTraceMap != null)
						&& (instructionParser.getAffectedRegisters() != null)) {
					if (DEBUG_FLOW)
						System.out.println("Flow: before saveRegTraceMap");
					saveRegTraceMap(instructionParser, regTraceMap);
					if (DEBUG_FLOW)
						System.out.println("Flow: after saveRegTraceMap");
				}

				// Mark that we have visited this place
				int instructionEndPos = (int) (instructionParser
						.getFilePosition() - startPos);
				visitSet.set(basePos, instructionEndPos);

				// Determine, where to continue the tracing
				DexInstructionParser.ForkStatus forkStatus = instructionParser
						.getForkStatus();
				if (DEBUG_FLOW)
					System.out.println("Flow: forkStatus: " + forkStatus);
				...
				if ((forkStatus == DexInstructionParser.ForkStatus.FORK_UNCONDITIONALLY)
					...
					int baseIndex = forkStatus == DexInstructionParser.ForkStatus.FORK_UNCONDITIONALLY ? 1
							: 0;
					long forkData[] = instructionParser.getForkData();
					// Mark the jump target locations that they are target of
					// jump instructions
					if (registerMaps != null) {
					...
					// we go to forkData[0], push the rest of the addresses to
					// the visit stack
					for (int i = baseIndex; i < forkData.length; ++i) {
						long target = forkData[i];
						if (DEBUG_FLOW)
							System.out.println("Flow: processing forkData[" + i
									+ "]: target: 0x"
									+ Long.toHexString(target));
						if ((target >= startPos) && (target <= endPos)) {
							HashMap<Integer, String> currentRegMap = instructionParser
									.getRegisterMap();
							HashMap<Integer, String> clonedCurrentRegMap = (HashMap<Integer, String>) currentRegMap
									.clone();
							visitStack.push(new VisitStackEntry(target,
									clonedCurrentRegMap));
						}
					}
					if (forkStatus == DexInstructionParser.ForkStatus.FORK_UNCONDITIONALLY)
			...
			if (DEBUG_FLOW)
				System.out.println("Flow: block parsing exit"
						+ Long.toHexString(filePos));
			// Branch ended (either by reaching end of method or hitting a
			// previously visited instruction)
			...
			// If this is an exception handler entry point, we should have a
			// saved
			// register map for it.
			if (DEBUG_EXCP)
			...
			if (DEBUG_FLOW)
				System.out.println("Flow: iteration, target address: "
						+ Long.toHexString(target));
		...
		if (debugInfoParser != null)
			processDebugInfoBlock(debugInfoParser, instructionParser,
					dexMethodHeadParser);
		...
		while ((actualPosition = instructionParser.getFilePosition()) < endPos) {
			DedexerTask task = instructionParser
					.getTaskForAddress(actualPosition);
			...
			if (!parseFlag) {
				// Let's check whether the first pass visited this region. If
				// not, turn it into data block
				...
				if (visitSet.get(visitOffset)) {
					...
					// We have run into an unvisited block. Turn it into a byte
					// dump
					String label = DexInstructionParser
							.labelForAddress(instructionParser
									.getFilePosition());
					openDataArray(label);
					StringBuilder element = new StringBuilder();
					boolean firstByte = true;
					while (((actualPosition = instructionParser
							.getFilePosition()) < endPos)
							&& (!visitSet.get(visitOffset++))) {
						task = instructionParser
								.getTaskForAddress(actualPosition);
  ...
  private void processTryCatchBlock( 
    ...
    for( int i = dtcb.getTriesSize() - 1 ; i >= 0  ; --i ) {
      ...
      for( int n = 0 ; n < dtcb.getTryHandlersSize( i ) ; ++n ) {
        ...
        int typeIdx = dtcb.getTryHandlerTypeIdx( i,n );
        ...
        instructionParser.
        getCodeGenerator().
        writeTryCatchBlockWithFinallyType(typeIdx, startLabel,endLabel,excpType,handlerOffsetName );
      }
    }
  ...
  private void processDebugInfoBlock(
    ...
    for( int i = 0 ; i < ddp.getLocalVariables() ; ++i ) {
      ...
      if( regNum >= 0 ) {
        ...
        //we now don't use this information
/*        writeLocalVariable( regNum,
      ...
      if( annotationIdx >= 0 ) {
        ...
        for( int n = 0 ; 
          ...
          String type = dap.getAnnotationType( 
              DexAnnotationParser.AnnotationType.METHOD,
              annotationIdx, n );
         // System.out.println("in method annotation: type is\n " + type);
         // System.out.println("in method annotation: type is " +  SExpHelpers.LTypeToSXType( type ));
          if( !DexAnnotationParser.isSystemAnnotation( type ) ) {
            ps.println( "  (annotation "+
                getVisibility( visibility )+
                " "+
  ...
  private void printElements( PrintStream ps, 
      ...
      int annotationIdx,
      int n ) {
	//  System.out.println("in printElement annotation");
    for( int k = 0 ; k < dap.getAnnotationElementsSize( 
        type,
        annotationIdx,n ) ; ++k ) {
      String parmName = dap.getAnnotationElementName( 
      ...
      ps.println( "    ("+
          ...
          SExpHelpers.LTypeToSXType( DexEncodedArrayParser.getTypeString( o ) )+
          " "+
          processAnnotationElementValue(o)+
          ")" );
    }
  }
  
  private String processAnnotationElementValue(Object o) {
	if(o instanceof StaticArray) {
		StaticArray sa = (StaticArray)o;
		StringBuilder b = new StringBuilder();
           // b.append( "( " );
            for( int i = 0 ; i < sa.length() ; ++i ) {
                if( i > 0 )
                    b.append( "  " );
                b.append(SExpHelpers.LTypeToSXType(sa.get(i).toString()) );
            }
          //  b.append( " )" );
			return b.toString();
	}
      ...
      if( "value".equals( elementName ) ) {
        ...
        if( o instanceof StaticArray ) {
          ...
          System.err.println("what" +
          		"s the array????? why there is no throws?"+ array);
          for( int n = 0 ; n < array.length() ; ++n )
            ps.println( "  (throws "+SExpHelpers.LTypeToSXType((String)array.get( n ))+")" );
        }
      }
      else{
    	  System.err.println("value is not equal to " + elementName);
      }
    }
  }

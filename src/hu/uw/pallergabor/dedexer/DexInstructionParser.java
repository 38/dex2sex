...
// index is encoded on the lowest 4 bit of the regno byte
                ...
                //instrText.append( "{" );
                instrText.append("invoke-static {");
                int regByte = 0;
                int byteCounter = 0;
                ...
                instrText.append(") [object java/lang/reflect/Array]");
                regMap.put( REGMAP_RESULT_KEY,
...
// point tracing registers if the invoke-quick result types cannot be calculated.
            ...
            case FILLEDARRAY_RANGE: {
            	...
            	int rangeend = rangestart + regno - 1;
            	instrText = new StringBuilder();
            	instrText.append("invoke-static/range ");
            	if(regno == 1)
            	{
...
// points to the static array data used to fill the array
            case FILLARRAYDATA: {
                ...
                instrText = new StringBuilder();
                instrText.append( "invoke-static {v"+
                        reg+
                        "} java/lang/reflect/Array/<fill_array_data> "+
                        "([object <adam_label>/l"+Long.toHexString( target ) + "]) void" );
...
// to that of the check cast type.
            case CHECKCAST: {
                ...
                if( !castType.startsWith( "[" ) )
                  castType = "L" + castType + ";";
                instrText.append( "v"+reg+
                                " "+SExpHelpers.LTypeToSXType(castType));
                affectedRegisters = new int[1];
                affectedRegisters[0] = reg;
...
// 16 bit type index follows. The register is associated with that type
            case NEWINSTANCE: {
                int reg = read8Bit();
                int typeidx = read16Bit();
                String type = SExpHelpers.LTypeToSXType(dexTypeIdsBlock.getClassName( typeidx ));
                instrText.append( "v"+reg+
                                " "+type );
                affectedRegisters = new int[1];
                affectedRegisters[0] = reg;
...
// high and low 4-bits. Then a 16 bit type index follows.
            case TWOREGSTYPE: {
                ...
                instrText.append( "v"+reg1+
                                " v"+reg2+
                                " "+ SExpHelpers.LTypeToSXType(dexTypeIdsBlock.getClassName( typeidx )) );
                affectedRegisters = new int[2];
                affectedRegisters[0] = reg1;
                affectedRegisters[1] = reg2;
                regMap.put( new Integer( reg1 ),TYPE_SINGLE_LENGTH );
...
// register overwrites the first
            case MOVE_OBJECT: {
                ...
                regMap.put( new Integer( reg1 ), getRegType( instrBase,reg2 ));
                		//SExpHelpers.LTypeToSXType(getRegType( instrBase,reg2 )) );
            }
            break;
            ...
            case TWOREGSPACKED_SINGLE: {
                ...
                else
                {
                	...
                	instrText.append("_invoke-v" + reg1 + "-static {v" + reg2 + "} "
                			         + "java/lang/reflect/Array/<array_length> ([object java/lang/reflect/Array]) int");
                }
                affectedRegisters = new int[2];
                affectedRegisters[0] = reg1;
                affectedRegisters[1] = reg2;
